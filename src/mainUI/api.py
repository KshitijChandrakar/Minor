...
def filesChange(request):
    ...
    try:
        ...
        for idx, change in enumerate(changes):
            if not (isinstance(change, dict) and 
                    isinstance(change.get('filename'), str) and 
                    isinstance(change.get('newContent'), str)):
                raise ValueError("WRONG WRONG WRONG")
    except ValueError:
        return JsonResponse({
            'error': f'NoNoNononOnonoNOONNOOOOOOO WRONG REQUEST'
        }, status=400)
    ...
    user.save(update_fields=['file_store'])
